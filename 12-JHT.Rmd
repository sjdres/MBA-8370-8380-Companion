...
* benchmark... all schools ranked 26 or higher!
...
```{r,eval = FALSE}
REG <- lm(SALARY~LSAT+TOP10+R11_25, data = LAW)
summary(REG)
```
...
$$\frac{\Delta \widehat{SALARY}}{\Delta TOP10} = `r tB2`$$

Holding LSAT constant, graduating from a top 10 school (relative to a school ranked 26 or lower) will result in $`r tB2` in additional median starting salary, on average.

$$\frac{\Delta \widehat{SALARY}}{\Delta R11\_25} = `r tB3`$$

Holding LSAT constant, graduating from a school ranked between 11-25 (relative to a school ranked 26 or lower) will result in $`r tB3` in additional median starting salary, on average.

The big difference between this regression model and the one we estimated in our dummy variable chapter is that the previous model implicitly assumed that the impact of graduating from a top 10 or top 11-25 school was the same (because we only considered top 25). This model relaxed that assumption and considered two *potentially* different impacts. The question remains: is this refinement necessary? In other words, is there a significant difference between top 10 and top 11-25 in the population?
...
plot(X,Y,type="n",ylab = "Density", xlab = " ",
     yaxt = "n", xaxt = "n", main = "F Distribution")
  xtick<-c(0, 3)
  axis(side=1, at=xtick, labels = FALSE)
  text(x=xtick, par("usr")[3],  
     labels = c(0,round(Fstat,2)), 
     pos = 1, xpd = TRUE)
  i <- X >= 0 & X <= 3
  i2 <- X >= 3 & X <= 4
  lines(X, Y)
  polygon(c(0,X[i],3), c(0,Y[i],0), col="orchid")
  polygon(c(3,X[i2],4), c(0,Y[i2],0), col="cyan")
```

Step 4: Calculate a p-value and conclude

```{r}
(P = pf(Fstat,m,n-k-1,lower.tail = FALSE))
(1-P)*100
```

Our p-value states that we can reject the null with `r round((1-P)*100,2)` percent confidence.
